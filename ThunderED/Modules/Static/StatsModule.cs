    ...
    internal class StatsModule: AppModuleBase
    {
        ...
        public static async Task Stats(ICommandContext context, string commandText)
        {
            ...
            if (commandText.ToLower() == "t" || commandText.ToLower() == "today" || commandText.ToLower() == "newday")
            {
                ...
                var losses = await RequestAsync<List<ZkbResponse.ZkbKill>>(requestHandler, new Uri(new Uri("https://zkillboard.com"), relPath)) ?? new List<ZkbResponse.ZkbKill>();
                relPath = "/api/kills";
                relPath = options.GetQueryString(relPath);
                var kills = await RequestAsync<List<ZkbResponse.ZkbKill>>(requestHandler, new Uri(new Uri("https://zkillboard.com"), relPath)) ?? new List<ZkbResponse.ZkbKill>();
            ...
            else
            {
                ...
                if (commandText.ToLower() == "month" || commandText.ToLower() == "m")
                {
                    ...
                    if (data == null) return;
                    await APIHelper.DiscordAPI.ReplyMessageAsync(context, $"{string.Format(LM.Get("monthlyStats"), result.Info.Name)}**:\n{LM.Get("Killed")}:        \t**{data.ShipsDestroyed}** ({data.IskDestroyed:### ### ### ### ###} ISK)\n{LM.Get("Lost")}: \t**{data.ShipsLost}** ({data.IskLost:### ### ### ### ###} ISK)");
                }
                else if (commandText.ToLower().All(char.IsDigit))
                {
                    ...
                    await APIHelper.DiscordAPI.ReplyMessageAsync(context, $"{string.Format(LM.Get("yearlyStats"),result.Info.Name,commandText)}:\n{LM.Get("Killed")}:        \t**{shipsDestroyed}** ({iskDestroyed:### ### ### ### ###} ISK)\n{LM.Get("Lost")}: \t**{shipsLost}** ({iskLost:### ### ### ### ###} ISK)");
                }
                else if (commandText.ToLower().Contains("/"))
                {
                    ...
                    var iskDestroyed = list.Sum(a => a.Value.IskDestroyed);
                    var iskLost = list.Sum(a => a.Value.IskLost);
                    await APIHelper.DiscordAPI.ReplyMessageAsync(context, $"{string.Format(LM.Get("monthlyCustomStats"),result.Info.Name,commandText)}:\n{LM.Get("Killed")}:        \t**{shipsDestroyed}** ({iskDestroyed:### ### ### ### ###} ISK)\n{LM.Get("Lost")}: \t**{shipsLost}** ({iskLost:### ### ### ### ###} ISK)");
                }
            }
        }
