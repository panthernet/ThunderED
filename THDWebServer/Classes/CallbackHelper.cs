...
namespace THDWebServer.Classes
{
    ...
    {
        public static async Task ProcessCallbackReply(NavigationManager manager, ProtectedSessionStorage store, CustomAuthenticationStateProvider auth, string request, string ip, CallbackTypeEnum cType)
        {
            ...
            if (request.Contains("&state=userauth"))
            {
                ...
                else
                {
                    ...
                    var r = await WebAuthModule.GetCharacterIdFromCode(code, SettingsManager.Settings.WebServerModule.CcpAppClientId,
