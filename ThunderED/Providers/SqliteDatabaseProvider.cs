    ...
    internal class SqliteDatabaseProvider : DBProviderBase<SqliteConnection, SqliteCommand>, IDatabasePovider
    {
        ...
        public async Task<List<object[]>> SelectData(string table, string[] fields, Dictionary<string, object> where = null)
        {
            ...
            return await SessionWrapper(query, async command =>
            {
                ...
                {
                    await LogHelper.LogEx($"[SelectData]: {query} - {string.Join("|", where?.Select(a=> $"{a.Key}:{a.Value} ") ?? new List<string>())}", ex, LogCat.Database);
                    return default;
                }
        ...
        public async Task<T> Query<T>(string table, string field, Dictionary<string, object> where)
        {
            ...
            return await SessionWrapper(query, async command =>
            {
                ...
                {
                    await LogHelper.LogEx($"[Query]: {query} - {string.Join("|", where.Select(a=> $"{a.Key}:{a.Value} "))}", ex, LogCat.Database);
                    return default;
                }
        ...
        {
            var field = string.Join(',', fields);
            var query = $"SELECT {field} FROM {table} WHERE authState=2 and main_character_id is null and (`{whereField}` is null or `{whereField}` <= datetime('now','-{minutes} minutes','localtime')) LIMIT {limit}";
